...
class BytecodeProcessor {
  ...
  /**
   * Generate realistic test arguments for common contract types
   * @param {Array} abi - Contract ABI
   * @param {string} contractType - Detected contract type
   * @returns {Object} Function signatures with generated arguments
   */
  generateTestArguments(abi, contractType) {
    const testArgs = {};
    
    for (const func of abi.filter(item => item.type === 'function')) {
      const signature = `${func.name}(${func.inputs.map(input => input.type).join(',')})`;
      testArgs[signature] = this.generateArgsForFunction(func, contractType);
    }
